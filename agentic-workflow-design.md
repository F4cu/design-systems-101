...
The fourth pattern, what design-system-ops calls the **feedback loop** — pairing a generator with a reviewer that sends work back — is different from the other three in one important way: it's the only one of the four with no built-in stopping point. This wiki calls it by its more specific name, a **generator/reviewer loop**, one type of a broader category — the **generative loop** — covered on its own page for that reason: [Generative loops](feedback-loops.md).
