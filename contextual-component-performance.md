...
The instrumentation gap described above — product analytics is context-aware but component-blind, component analytics is usage-aware but context-blind — has historically required stitching together two separate vendors (a product-analytics tool and a component/design-tooling scanner) by hand. Platforms like [PostHog](https://posthog.com/docs/llm-analytics) have recently started unifying product analytics (funnels, session replay, feature flags) with LLM/agent observability (traces, evaluations, cost and latency per model call) in a single system, where "every trace has a person behind it" — meaning an LLM call and the human session around it already share one identity graph, natively. That's a materially different starting point than wiring two disconnected tools together, and it opens up a few concrete possibilities worth naming even though nobody has written them up as a pattern yet:
