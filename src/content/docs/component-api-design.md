...
Sharing "tokens and intent" across platforms only works if that intent is recorded somewhere neutral in the first place — see [Multi-platform component specs](/multi-platform-component-specs/) for how that source of truth is structured once more than one independent platform team is building from it, and [Platform divergence](/platform-divergence/) for why a structural split like Dialog vs. BottomSheet needs a component contract rather than a token.
