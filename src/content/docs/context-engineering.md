...
Wolosin didn't assume a metadata format would work. She tested it. At Indeed, she benchmarked eight MCP (Model Context Protocol) configurations against 1,056 prompts to find which structure agents actually resolved correctly. The "obviously correct" format isn't obvious until you measure it. [Generative loops](/feedback-loops/) covers the same propose-measure-revise discipline in agent orchestration, where the harder question is when the loop stops.
