...
- **Confusing multi-platform with cross-platform.** The goal isn't one implementation forced to look identical everywhere — see [Component API design](/component-api-design/) on Wealthfront's "design once, build anywhere" — it's one set of decisions expressed idiomatically per platform, which still requires that the *decisions* live somewhere neutral.
