...
That rating is usually treated as fixed per component. It isn't: the same component's effective CR shifts with where it's placed — see [Performance in context](/contextual-component-performance/).
...
- **GitHub** — reviews PRs and diffs token definitions to catch design-code drift, the same failure mode covered in [Dependency observability](/dependency-observability/)
- **GitLab** — manages issues, merge requests, and CI pipelines through AI workflows
- **PostHog** — checks design decisions against real adoption and conversion data, the same evidence [Adoption & measurement](/adoption-measurement/) argues for
